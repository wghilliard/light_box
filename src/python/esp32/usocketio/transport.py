...
class SocketIO:
    ...
    def __init__(self, uri, reconnect=True, timeout=1, **params):
        self.uri = uri
    ...
    def _handle_event(self, event, data=None):
        ...
        for handler in self._event_handlers.get(event, []):
            # print("Calling handler for event {0}".format(event))
            handler(data)
    ...
    def _send_message(self, message_type, data=None):
        self._send_packet(PACKET_MESSAGE, "{}{}".format(message_type, json.dumps(data)))
    ...
    def on(self, event):
        ...
        def inner(func):
            # print("Registered {0} to handle {1}".format(func, event))
